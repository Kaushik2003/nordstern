...
export default function PricingPage() {
  ...
  const backtest = useMemo<CurvePoint[]>(() => {
    ...
    for (let s = PRICING.offramp.min; s <= PRICING.offramp.max + 1e-9; s += 0.1) {
      const vol = monthlyOut * relVol(s, PRICING.offramp.market);
      pts.push({ s: +s.toFixed(2), vol, earned: vol * (s / 100) });
    }
    return pts;
  }, []);

  const offEarned = earnedSide(monthlyOut, offramp, PRICING.offramp.market);
  const tradeoff = ((relVol(offramp, PRICING.offramp.market) - relVol(PRICING.offramp.rec, PRICING.offramp.market)) / relVol(PRICING.offramp.rec, PRICING.offramp.market)) * 100;
  const totalEarned = earnedSide(monthlyIn, onramp, PRICING.onramp.market) + offEarned;

  if (!ready) return <PricingSkeleton />;

  return (
    <PageContainer>
      <PageHeader title="Pricing & Liquidity" subtitle="Set what you charge. NordStern recommends the optimal spread from live conditions — you decide." />

      {/* On-ramp & off-ramp */}
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardBody>
            <CardHead label="On-ramp · INR → INRT" info="The spread you take when users deposit and you mint INRT." />
            <div className="mt-4">
              <SpreadControl
                value={onramp}
                onChange={(v) => setOnramp(v)}
                min={PRICING.onramp.min}
                max={PRICING.onramp.max}
                rec={PRICING.onramp.rec}
                market={PRICING.onramp.market}
                zones={{ good: 1.1, warn: 1.8 }}
                presets={{ conservative: 0.7, recommended: PRICING.onramp.rec, aggressive: 1.3 }}
              />
            </div>
        ...
        <Card>
          <CardBody>
            <CardHead label="Off-ramp · INRT → INR" info="The spread you take when users redeem INRT and you pay out fiat." />
            <div className="mt-4">
              <SpreadControl
                value={offramp}
                onChange={(v) => setOfframp(v)}
                min={PRICING.offramp.min}
                max={PRICING.offramp.max}
                rec={PRICING.offramp.rec}
                market={PRICING.offramp.market}
                zones={{ good: 1.5, warn: 2.2 }}
                presets={{ conservative: 1.2, recommended: PRICING.offramp.rec, aggressive: 2.0 }}
              />
            </div>
            <WhyLine>USDC volatility is up and your book is short USDC — a wider spread protects margin.</WhyLine>
          </CardBody>
        </Card>
      </div>

      {/* Fee structure · inventory · guardrails */}
          ...
          <CardBody>
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center justify-between rounded-[10px] border border-border-subtle px-3 py-2.5">
                ...
                <span className="font-mono text-[12.5px] tabular-nums text-text-primary">{PRICING.offramp.min.toFixed(1)}–{PRICING.offramp.max.toFixed(1)}%</span>
              </div>
      ...
      <Card className="mt-4">
        <CardBody>
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <button
              onClick={() => { setOfframp(PRICING.offramp.rec); toast("Applied recommended off-ramp spread", { description: `${PRICING.offramp.rec.toFixed(2)}%` }); }}
              className="text-[12px] font-medium text-brand hover:text-brand-300"
            >
