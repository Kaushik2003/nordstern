...
import { users as ALL, KYC_FUNNEL, ACTIVE_USERS } from "@/lib/data/store";
...
export default function UsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return ALL.filter((u) => {
      ...
      if (query && !`${u.name} ${u.city} ${u.address}`.toLowerCase().includes(query)) return false;
      return true;
    });
  }, [status, tier, q]);
    ...
    <PageContainer>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        ...
        <div className="grid content-start gap-4">
          ...
          <Card>
            <CardBody>
              <CardHead label="Total users" />
              <div className="mt-2 font-mono text-[24px] font-semibold tabular-nums text-text-primary">{groupIN(ACTIVE_USERS)}</div>
              <div className="mt-1 text-[12px] text-text-tertiary">across all tiers</div>
            </CardBody>
          </Card>
        </div>
      </div>
      ...
      <div className="mb-3 mt-4 flex flex-wrap items-center gap-2">
        ...
        <SearchInput placeholder="Search name, city, address…" value={q} onChange={(e) => setQ(e.target.value)} className="ml-auto w-full sm:w-64" />
      </div>
              ...
              <VirtualRows
                ...
                renderRow={(u) => (
                  <button onClick={() => { setSelected(u); setDrawer(true); }} className={cn(COLS, "h-full w-full border-b border-border-subtle text-left transition-colors hover:bg-surface-2/50")}>
                    ...
                    <span><Pill tone={riskTone[u.risk]}>{u.risk[0].toUpperCase() + u.risk.slice(1)}</Pill></span>
                  </button>
                )}
              />
            )}
