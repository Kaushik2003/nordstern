...
> "Anchor Network" phase of the platform architecture docs and the `nordstern-platform-vision`
...
|---|---|---|---|---|---|
| **NGINX Ingress** | Reload-based; annotation sprawl; large-config reloads hurt at fleet scale | via cert-manager | OK | Weak (annotations) | Works, but reload model + the community controller's uncertain long-term future are strikes. |
...
| **Istio ingress** | | via cert-manager | | but heavy | Only if you're already running the full mesh; otherwise operational overkill. |
