...
import { TREASURY, withdrawals as seedWithdrawals } from "@/lib/data/store";
...
export default function TreasuryPage() {
  ...
  const [balance, setBalance] = useState(TREASURY.available);
  ...
  const [history, setHistory] = useState<WithdrawalRecord[]>(seedWithdrawals);

  const animatedBalance = useCountUp(balance, { duration: 900 });
  ...
  const onComplete = (r: WithdrawResult) => {
    ...
    setHistory((h) => [rec, ...h]);
    setTimeout(() => {
      setHistory((h) => h.map((x) => (x.id === rec.id ? { ...x, status: "settled" } : x)));
    }, 2200);
  };

  const tierTotal = tiers.hot + tiers.warm + tiers.deployable;

  if (!ready) return <TreasurySkeleton />;

  return (
    <PageContainer>
      <PageHeader title="Treasury" subtitle="What you've made, the reserves backing your tokens, and your money to withdraw." actions={<Pill tone="brand">Testnet</Pill>} />
        ...
        <CardBody className="relative grid items-center gap-6 py-7 md:grid-cols-[1fr_auto] md:py-9">
            ...
            <div className="mt-3 flex items-center gap-2 text-[13px]">
              {balance > 0 ? (
                ...
                <>
                  ...
                  <span className="text-text-secondary">Withdrawn — funds en route to Acme Pay ••6642</span>
                </>
              )}
            </div>
          </div>
          <div className="flex flex-col items-stretch gap-2 md:items-end">
            ...
            <p className="text-center font-mono text-[11.5px] text-text-tertiary md:text-right">
              {inr(TREASURY.pending)} pending · {TREASURY.pendingCount} settlements in flight
            </p>
          </div>
        </CardBody>
      </Card>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        <Kpi label="Pending settlements" info="In-flight deposits not yet swept into available balance." value={TREASURY.pending} render={(n) => inr(n)} sub={`${TREASURY.pendingCount} in-flight deposits`} />
        <Kpi label="Lifetime volume" info="Gross value moved through the anchor since inception." value={TREASURY.lifetimeVolume} render={(n) => inrCompact(n)} />
        <Kpi label="Earned · 30D" info="Your revenue: fees + spread + yield over the last 30 days." value={TREASURY.earned30d} render={(n) => money(Math.round(n))} delta={TREASURY.earned30dDelta} accent="text-pos" />
      </div>
      ...
      <Card className="mt-4">
        <CardBody className="grid items-center gap-6 md:grid-cols-[auto_1fr_auto]">
          <div className="mx-auto">
            <ReserveGauge ratio={TREASURY.ratio} size={170} />
          </div>
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-3">
            <Stat label="Tokens issued" value={`${groupIN(TREASURY.tokensIssued)}`} unit="INRT" />
            <Stat label="Reserves" value={inr(TREASURY.reserves)} />
            <Stat label="Backing ratio" value={`${TREASURY.ratio.toFixed(1)}%`} accent="text-pos" />
          </div>
          <div className="flex flex-col items-stretch gap-2">
            <Button variant="secondary" leadingIcon={<FileText className="size-4" />} onClick={() => setAttestOpen(true)}>
              View attestation
            </Button>
            <p className="text-center font-mono text-[10.5px] text-text-tertiary">Last verified {relTime(TREASURY.lastVerified, now)}</p>
          </div>
        </CardBody>
      </Card>
      ...
      <WithdrawFlow open={withdrawOpen} onOpenChange={setWithdrawOpen} available={balance} onComplete={onComplete} />
