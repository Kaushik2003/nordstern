...
import { cases as seedCases, sanctions as seedSanctions, monitoringRules as seedRules, auditLog, COMPLIANCE_KPIS, type ComplianceCase, type CaseStatus, type SanctionMatch, type MonitoringRule } from "@/lib/data/compliance";
...
export default function CompliancePage() {
  ...
  // Demo cue (scenario §8): jump to the audit log and verify integrity.
  useEffect(() => {
  ...
  if (!ready)
    return (
      <PageContainer>
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-24 rounded-[14px]" />)}</div>
  ...
  return (
    <PageContainer>
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi label="Open cases" value={String(COMPLIANCE_KPIS.openCases)} tone="text-warn" />
        <Kpi label="Sanctions hits · 24h" value={String(COMPLIANCE_KPIS.sanctionsHits24h)} tone="text-crit" />
        <Kpi label="STRs filed · MTD" value={String(COMPLIANCE_KPIS.strsMTD)} />
        <Kpi label="Travel-Rule coverage" value={`${COMPLIANCE_KPIS.travelRuleCoverage}%`} tone="text-pos" />
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabContent value="cases"><CasesTab /></TabContent>
        <TabContent value="screening"><ScreeningTab /></TabContent>
        <TabContent value="rules"><RulesTab /></TabContent>
        <TabContent value="audit"><AuditTab autoVerify={autoVerify} /></TabContent>
      </Tabs>
    </PageContainer>
  );
}
...
function CasesTab() {
  const [cases, setCases] = useState<ComplianceCase[]>(seedCases);
  const [sel, setSel] = useState<ComplianceCase | null>(null);
  const [open, setOpen] = useState(false);
  const resolve = (id: string, status: CaseStatus) => setCases((cs) => cs.map((c) => (c.id === id ? { ...c, status } : c)));
    ...
    <Card className="overflow-hidden p-0">
      <div className="overflow-x-auto">
        <div className="min-w-[820px]">
          ...
          {cases.map((c) => (
            <button key={c.id} onClick={() => { setSel(c); setOpen(true); }} className="grid w-full grid-cols-[110px_minmax(140px,1fr)_minmax(180px,1.4fr)_90px_120px_110px] items-center gap-2 border-b border-border-subtle px-5 py-3 text-left transition-colors hover:bg-surface-2/50">
...
function ScreeningTab() {
  ...
  return (
    <div className="grid gap-4">
      ...
      <Card>
        <CardBody>
          <CardHead label="Travel Rule" info="Originator & beneficiary info attached on transfers above ₹50,000." />
          <div className="mt-3 flex items-center gap-4">
            <div className="flex-1">
              <div className="h-2 overflow-hidden rounded-full bg-surface-3"><div className="h-full rounded-full bg-pos" style={{ width: `${COMPLIANCE_KPIS.travelRuleCoverage}%` }} /></div>
            </div>
            <span className="font-mono text-[18px] font-semibold tabular-nums text-pos">{COMPLIANCE_KPIS.travelRuleCoverage}%</span>
          </div>
          <p className="mt-2 text-[12.5px] text-text-secondary">Coverage of qualifying transfers carrying full originator/beneficiary data.</p>
        </CardBody>
      </Card>
    </div>
  );
...
function AuditTab({ autoVerify = false }: { autoVerify?: boolean }) {
    ...
    <Card className="overflow-hidden">
      <CardBody className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="secondary" leadingIcon={<ShieldCheck className="size-3.5" />} onClick={verify}>Verify integrity</Button>
          <Button size="sm" variant="primary" leadingIcon={<FileText className="size-3.5" />} onClick={() => toast("Compliance report generated", { description: "Period · policy checks passed · ratios · case outcomes" })}>Generate report</Button>
        </div>
      </CardBody>
